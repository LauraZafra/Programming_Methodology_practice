...
@author estudiante1: Susana Salvatierra Fernández
@author estudiante2: Laura Zafra Alarcos
...
public:
     ...
     */
    void extractObjects(Image et [], int &nimages, int maximages, double tolerance=Histogram::HISTOGRAM_TOLERANCE) const;
    /**
    ...
    */
    ...
    Image zoomIn()const;
    /**
     * @brief It zooms the image out. Query method
     * @return The zoomed image
    */
    Image zoomOut() const;
    ...
    bool hideText(char source [], int k);
    
    bool showText(char hidden [], const int max, int k =0) const;
...
private:
    ...
    //Pasa los datos de una función a otra 
    void copyData(const Image &from);
    
    // Función auxiliar para reservar memoria
    Byte** reservarM(int w,int h);
    
    Byte extractByte(int &pos_act, int k) const;
    
    std::string extractString (int &pos_act, int k) const;
    
    void hideId(char a, char b, char c, int k);
    
    void hideSize(int desde_donde, int byte, int k);
    
    void setBit(Byte b, int i, int fil, int col, int k);
    
    bool sameId(Byte a1, char a2, Byte b1, char b2, Byte c1, char c2)const;
